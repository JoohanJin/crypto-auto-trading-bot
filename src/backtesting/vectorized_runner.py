...
class HighResPeakOptimizer:
    ...
    def load_and_signals(self):
        ...
        df['c_short'] = rolling_consensus(sig_scores, 120)  # 2 minutes
        df['c_mid'] = rolling_consensus(sig_scores, 300)    # 5 minutes
        df['c_struct'] = rolling_consensus(sig_scores, 600) # 10 minutes
        df['density'] = pd.Series(sig_scores != 0).rolling(window=600).sum().values # 10 minutes
    ...
    def run(self):
        ...
        combos = list(itertools.product(c_shorts, c_mids, c_structs))
        print(f"[2/2] High-Res Grid Search: {len(combos)} combinations...")

        results = []
        start = time.time()
        for cs_val, cm_val, cst_val in combos:
            config = {'c_short': cs_val, 'c_mid': cm_val, 'c_struct': cst_val}

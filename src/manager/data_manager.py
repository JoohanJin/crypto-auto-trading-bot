...
class DataManager:
    ...
    def __init__(
        ...
        self.price_data: pd.DataFrame = pd.DataFrame(
            ...
            index = [DataManager.generate_timestamp()],
        )
    ...
    def __resize_df(
        ...
        return None
        """
        curr_timestamp = DataManager.generate_timestamp()
        while True:
            data = None
            try:
                if (DataManager.generate_timestamp() - curr_timestamp > (wait_time * 1_000)):  # Five minutes
                    with self.df_lock:
                        if self.price_data.shape[0] > self._df_size_limit:
                            ...
                            operation_logger.info(
                                f"{__name__} - {self.__class__.__name__} - Data Saver skipped cleanup; size below threshold: "
